...
import { characterState, createSession, splitCharacters, updateSession } from "./engine.js";
...
function install() {
  ...
  function startPrompt(advance) {
    window.clearTimeout(state.nextTimer);
    const text = !advance && state.promptIndex >= 0
      ? CORPUS[state.promptIndex]
      : selectPrompt(state.promptIndex);
